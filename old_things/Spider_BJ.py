...
#     html = file
#     Carbon_BJ = pd.DataFrame(columns=['Time', 'Number','Ave_Price','Total_cash'])
#     bs = BeautifulSoup(html, 'html.parser')
#     data = bs.find_all(class_='listtext')  # 注意这里是 bs.find_all() 而不是 BeautifulSoup.find_all(bs, ...)
#     for item in data:
#         temp = []
#         for row in item.find_all('tr'):
#             ttemp = []
#             for cell in row.find_all('td'):
#                 ttemp.append(cell.text)
#                 # print(cell.text)
#             if ttemp is not None:
#                 temp.append(ttemp)
#             else:
#                 pass
#         temp = pd.DataFrame(temp, columns=['Time', 'Number', 'Ave_Price', 'Total_cash'])
#         Carbon_BJ = pd.concat([Carbon_BJ, temp], ignore_index=True, axis=0)
#         print(Carbon_BJ)
...
if __name__ == '__main__':
    ...
    for i in range(1,stop):
        ...
        data = bs.find_all(class_='listtext')
        for item in data:
            temp=[]
            for row in item.find_all('tr'):
                ttemp = []
    ...
    print(Carbon_BJ)
    Carbon_BJ.to_excel('Carbon_BJ.xlsx')
